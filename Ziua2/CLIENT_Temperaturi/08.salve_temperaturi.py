## ACEASTA BUCATA DE COD serveste ca CITITOR
...
def save_to_csv(data):
    ...
    if pd.io.common.file_exists(CSV_FILE):
        ...
        mode = "w" # write
    print("new_df", new_df)
    has_header = not pd.io.common.file_exists(CSV_FILE)
    new_df.to_csv(CSV_FILE, mode=mode, index=False, header=has_header)
...
def function_that_executes_on_message(client, userdata, message):
    ...
    print(f"Mesajul primit:, {mesajul_transmis}")

    data = json.loads(mesajul_transmis)
